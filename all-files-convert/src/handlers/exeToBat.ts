...
export default class exeToBatHandler implements FormatHandler {
  ...
  supportedFormats: FileFormat[] = [
    ...
    CommonFormats.PYTHON.builder("py").allowTo().markLossless(),
  ];
  ...
  async doConvert(
    ...
    const toPython = outputFormat.mime === CommonFormats.PYTHON.mime;
    if (inputFormat.mime !== EXE_MIME || (!toPython && outputFormat.mime !== CommonFormats.BATCH.mime)) {
      if (DEBUG_EXE_TO_BAT)
        console.log(
          ...
          "or",
          CommonFormats.PYTHON.mime,
        );
      throw new TypeError("This handler only supports EXE to BAT or PY conversion");
    }

    const results: FileData[] = [];

    for (const file of inputFiles) {
      const result = toPython ? this.convertExeToPython(file) : await this.convertExeToBat(file);
      results.push(result);
    }
  ...
  private convertExeToPython(file: FileData): FileData {
    const exeName = file.name.replace(/\.[^.]*$/, "");
    // Wrapped at 76 columns, like the batch payload that certutil decodes.
    const base64 = Buffer.from(file.bytes).toString("base64").replace(/.{76}/g, "$&\n");
    return {
      name: `${exeName}.py`,
      bytes: new TextEncoder().encode(this.generatePythonWrapper(exeName, base64)),
    };
  }

  private generatePythonWrapper(exeName: string, payload: string): string {
    // JSON string literals are valid Python string literals.
    return `# Auto-generated EXE-embedded Python script
# Reconstructs and runs ${exeName}.exe
...
EXE_NAME = ${JSON.stringify(`${exeName}.exe`)}
...
def main():
    out_exe = os.path.join(tempfile.gettempdir(), EXE_NAME)
    with open(out_exe, "wb") as f:
        f.write(base64.b64decode(PAYLOAD))
    print("Reconstructed", out_exe)

    if sys.platform == "win32":
        os.startfile(out_exe)
    else:
        print("This is a Windows program; run it with Windows or Wine.")
