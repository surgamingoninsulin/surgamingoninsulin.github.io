...
export async function render(
  ...
  const sabers = [new Saber(leftMaterial), new Saber(rightMaterial)];
  const TIME_SCALE = 20;
  ...
  const dotGeometry = new THREE.CircleGeometry(NOTE_SCALE * 0.25).translate(
    0,
    0,
    NOTE_SCALE * 0.55,
  );
  ...
  class Note {
    ...
    constructor(data: BSOR.Note) {
      if (data.type === BSOR.CutType.BOMB) {
        ...
        this.bloq = new Mesh(
          noteGeometry,
          data.color === BSOR.NoteColor.LEFT ? leftMaterial : rightMaterial,
        );
      }
    ...
    update(time: number) {
      this.bloq.position.set(
        this.data.lineIndex - 1.5,
        this.data.lineLayer,
        TIME_SCALE * (time - this.data.spawnTime),
      );
