...
export class Replay {
  ...
  constructor(data: Uint8Array) {
    ...
    // magic number
    if (int() !== 0x442d3d69) raise("Invalid magic number");
    if (byte() !== 1) raise("Unknown BSOR version");
    if (byte() !== 0) raise("Expected info structure start");
    this.version = {
      mod: string(),
      game: string(),
    };
    ...
    this.frames = [];
    if (byte() !== 1) raise("Expected frame array start");
    ...
    if (byte() !== 2) raise("Expected note array start");
    this.notes = [];
    const noteCount = int();
    ...
    this.walls = [];
    if (byte() !== 3) raise("Expected wall array start");
    const wallCount = int();
    ...
    this.height = [];
    if (byte() !== 4) raise("Expected height array start");
    ...
    this.pause = [];
    if (byte() !== 5) raise("Expected pause array start");
