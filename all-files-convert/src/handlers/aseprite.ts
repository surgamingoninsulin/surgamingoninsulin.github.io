...
function decodeAseprite(bytes: Uint8Array): ParsedAseprite {
  ...
  if (magic !== ASEPRITE_HEADER_MAGIC)
    throw new BadMagicError(`Invalid Aseprite header magic: ${magic}`);
  ...
  if (width === 0 || height === 0)
    throw new RangeError(`Invalid image dimensions in Aseprite file: ${width}x${height}`);
  if (frameCount === 0) throw new RangeError("Aseprite file has no frames.");
  if (colorDepth !== 32 && colorDepth !== 16 && colorDepth !== 8) {
    throw new RangeError(
      `Unsupported Aseprite color depth of ${colorDepth}. Expected RGBA(32), Grayscale(16), or Indexed(8).`,
    );
  }
  ...
  for (let frameIndex = 0; frameIndex < frameCount; frameIndex++) {
    if (offset + 16 > bytes.length)
      throw new EOFError("Unexpected end of file while reading frame header.");
    const frameBytes = view.getUint32(offset, true);
    const frameMagic = view.getUint16(offset + 4, true);
    if (frameMagic !== ASEPRITE_FRAME_MAGIC)
      throw new BadMagicError(`Invalid frame magic in Aseprite file: ${frameMagic}`);
    ...
    for (let i = 0; i < chunkCount; i++) {
      if (chunkOffset + 6 > bytes.length)
        throw new EOFError("Unexpected end of file while reading chunk header.");
      ...
      if (chunkSize < 6 || chunkEnd > bytes.length || chunkEnd > frameEnd)
        throw new RangeError(`Invalid chunk size in Aseprite file: ${chunkSize}`);
      ...
      if (chunkType === CHUNK_TYPE_LAYER) {
        ...
        for (
          let colorIndex = firstColor;
          colorIndex <= lastColor && colorIndex < 256;
          colorIndex++
        ) {
        ...
        chunkType === CHUNK_TYPE_PALETTE_LEGACY_8BIT ||
        chunkType === CHUNK_TYPE_PALETTE_LEGACY_6BIT
      ) {
        ...
        for (let packet = 0; packet < packetCount && entryOffset + 2 <= chunkEnd; packet++) {
          ...
          for (
            let c = 0;
            c < colorCount && paletteIndex < 256 && entryOffset + 3 <= chunkEnd;
            c++
          ) {
        ...
        if (celType === 1) {
          ...
          if (celType === 0) {
            ...
            if (celBytes.length !== expectedByteLength)
              throw new RangeError(`Invalid decompressed cel size: ${celBytes.length}`);
          }
    ...
    if (chunkOffset > frameEnd)
      throw new RangeError(`Frame chunk data exceeds frame bounds: ${chunkOffset} > ${frameEnd}`);
  ...
  if (targetFrame.size === 0) {
    if (unsupportedCelTypes.size > 0) {
      throw new TypeError(
        `Unsupported Aseprite cel type(s): ${Array.from(unsupportedCelTypes).join(", ")}.`,
      );
    }
    ...
    for (let py = 0; py < cel.height; py++) {
      ...
      for (let px = 0; px < cel.width; px++) {
        ...
        const blend = (
          ...
          drgb: { r: number; g: number; b: number },
          channel: "r" | "g" | "b",
        ): number => {
          switch (mode) {
            case 1:
              return s * d; // multiply
            case 2:
              return 1 - (1 - s) * (1 - d); // screen
            case 3:
              return d <= 0.5 ? 2 * s * d : 1 - 2 * (1 - s) * (1 - d); // overlay
            case 4:
              return Math.min(s, d); // darken
            case 5:
              return Math.max(s, d); // lighten
            case 6:
              return s >= 1 ? 1 : Math.min(1, d / (1 - s)); // color dodge
            case 7:
              return s <= 0 ? 0 : 1 - Math.min(1, (1 - d) / s); // color burn
            case 8:
              return s <= 0.5 ? 2 * s * d : 1 - 2 * (1 - s) * (1 - d); // hard light
            case 9: {
              // soft light
            ...
            case 10:
              return Math.abs(d - s); // difference
            case 11:
              return s + d - 2 * s * d; // exclusion
            case 12: {
              // hue
            ...
            case 16:
              return Math.min(1, s + d); // addition
            case 17:
              return Math.max(0, d - s); // subtract
            case 18:
              return s === 0 ? 1 : Math.min(1, d / s); // divide
            default:
              return s; // normal and unsupported modes
          }
        };
