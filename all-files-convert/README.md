# [All Files Convert!](https://convert.to.it/)
...
1. Clone this repository _**WITH SUBMODULES**_. You can use `git clone --recursive https://github.com/surgamingonin/All-Files-Convert!` for that. Omitting submodules will leave you missing a few dependencies.
...
This runs the container on `http://localhost:8080/all-files-convert/`.
...
- If you need to load a WebAssembly binary (or similar), add its path to [vite.config.js](vite.config.js) and target it under `/all-files-convert/wasm/`. **Do not link to node_modules**.
